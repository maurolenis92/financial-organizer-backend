...
export class BudgetService {
  ...
  async createBudget(data: {
    ...
    const budget = await prisma.$transaction(async (tx) => {
      ...
      const totalIncomes = data.incomes ? data.incomes.reduce((sum, i) => sum + Number(i.amount), 0) : 0;
      const totalExpenses = data.expenses ? data.expenses.reduce((sum, e) => sum + Number(e.amount), 0) : 0;
...
async updateBudgetFull(budgetId: string, userId: string, data: UpdateBudgetFullDTO): Promise<Budget> {
...
private async syncValues(model: any, budgetId: string, values: IncomeDTO[] | CreateExpenseDTO[], isExpense: boolean, userId: string) {
